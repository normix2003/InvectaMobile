...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/inventario/detalle-categoria.css') }}" />
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Verifica si 'edit' está en localStorage, si no, lo establece en 'false'
            let isEdit = localStorage.getItem('editCategoria');
            if (isEdit === null) {
                localStorage.setItem('editCategoria', 'false');
            }
            ...
            if (localStorage.getItem('editCategoria') === 'false') {
            ...
            form.addEventListener('submit', function (event) {
                ...
                localStorage.removeItem('editCategoria'); // Limpiar el valor de 'edit' en localStorage
            ...
            if (editButton) {
                editButton.addEventListener('click', function (event) {
                    ...
                    localStorage.setItem('editCategoria', true);
                    location.reload();
                });
            }
...
<body>
    <header>
        <div class="header-container">
            ...
            <h1 class="header-title">Detalles de categoria</h1>
        </div>
        @if (session('success'))
    ...
    <main>
        ...
        @endif
        <form id="userForm" action="{{route('categoria.update', $categoria->idCategorias)}}" method="post">
            <div class="detalle-categoria-container">
                @php
                    $userPermisos = Auth::user()->rol->detallesroles->where('Eliminar', 0)->pluck('permisos.Nombre')->toArray();
                @endphp
                <h1 class="title">Informacion de la categoria seleccionada</h1>
                <div class="detalle-categoria-input">
                    @csrf
                    @method('PUT')
                    <input placeholder="Nombre categoria" name="Nombre_Categoria"
                        value="{{ $categoria->Nombre_Categoria}}" disabled Required></input>
