...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/inventario/detalle-marca.css') }}" />
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Verifica si 'edit' está en localStorage, si no, lo establece en 'false'
            let isEdit = localStorage.getItem('editMarca');
            if (isEdit === null) {
                localStorage.setItem('editMarca', 'false');
            }
            ...
            if (localStorage.getItem('editMarca') === 'false') {
            ...
            form.addEventListener('submit', function (event) {
                ...
                localStorage.removeItem('editMarca'); // Limpiar el valor de 'edit' en localStorage
            ...
            if (editButton) {
                editButton.addEventListener('click', function (event) {
                    ...
                    localStorage.setItem('editMarca', true);
                    location.reload();
                });
            }
            const regresar = document.getElementById('regresar');
            regresar.addEventListener('click', function (event) {
                localStorage.removeItem('editMarca');
                location.href = "{{ route('marcas') }}";
            });
        });
    </script>
</head>
...
<body>
    <header>
        <div class="header-container">
            ...
            <h1 class="header-title">Detalles de marca</h1>
        </div>
        @if (session('success'))
    ...
    <main>
        ...
        @endif
        <form id="userForm" action="{{route('marca.update', $marca->idMarcas)}}" method="post">
            <div class="detalle-marca-container">
                <h1 class="title">Informacion de la marca seleccionada</h1>
                <div class="detalle-marca-input">
                    @csrf
                    @method('PUT')
                    <input placeholder="Nombre marca" name="Nombre_Marca" value="{{ $marca->Nombre_Marca}}" disabled
                        Required></input>
